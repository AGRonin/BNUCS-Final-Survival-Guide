...
import subprocess
from 词云生成 import WordCloud_To_Image
...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        panel = ImagePanel(self, 'PieChart/PieChart_500*400.jpg')
...
class Direction_Sub_Page(wx.Frame):
    def __init__(self, parent, text, people_info_dictionary):
        ...
        people_info_list=list(people_info_dictionary.values())
        people_list=list(people_info_dictionary.keys())
        # 假设people_info是一个列表，每个元素包含(photo_path, bio_path, wordcloud_path,new_page_text_path)
        for i in range(0,len(people_info_list)):
            info=people_info_list[i]
            name=people_list[i]
            person_panel = PersonPanel(self.scrolled_window, name,*info)
...
class PersonPanel(wx.Panel):
    def __init__(self, parent,name,photo_path, bio_path, wordcloud_path,new_page_text_path):
        super().__init__(parent)
    ...
    def OnTextLeftDown(self, event):
        ...
        new_frame = wx.Frame(None, title=f'{self.name}', size=(1200, 900))
...
if __name__=="__main__":
    #python_WEB_path="/opt/anaconda3/envs/WEB"
    #spider_path='/爬虫/spider.py'
    #python_BNUSC_path='/opt/anaconda3/envs/BNUSC'
    #wordcloud_path='/词云生成/WordClout_To_Image.py'
    #subprocess.run([python_WEB_path,spider_path])
    #subprocess.run([python_BNUSC_path,wordcloud_path])
    #这里我们本来希望使用subprocess通过其他解释器运行其他代码，但mac系统报错：[Errno 13] Permission denied: '/opt/anaconda3/envs/BNUSC'
    #故我们在这里采用import函数的方法进行调用py文件
    PieChart.PieChart_function()

    #读取txt
    with open('人工智能学院各个研究方向导师.txt','r') as f:
        different_direction_teacher=f.read().splitlines()

    #导师信息列表
    direction_list=[]
    teacher_info_dictionary = {}
    for line in different_direction_teacher:
        direction_teacher=line.split(',')
        direction_list.append(direction_teacher[0])
        teacher_info_dictionary[direction_teacher[0]] = {}
        for teacher in direction_teacher[1:]:
            if len(teacher)>=1:
                absolute_path = '/Users/yangbowen/Desktop/本科/大一/大一下课程/Python程序设计/2024.6.27Python程序设计/大作业：智能导师推荐系统'
                photo_path=absolute_path+f'/爬虫/处理后数据_老师信息/{teacher}/pic.png'
                bio_path=absolute_path+f'/爬虫/处理后数据_老师信息/{teacher}/2.txt'
                WordCloud_To_Image.WordCloud_To_Image_function(absolute_path+f'/爬虫/处理后数据_老师信息/{teacher}')
                wordcloud_path=absolute_path+f'/爬虫/处理后数据_老师信息/{teacher}/WordCloud.png'
                new_page_text_path=absolute_path+f'/爬虫/处理后数据_老师信息/{teacher}/3.txt'
                teacher_info_dictionary[direction_teacher[0]][teacher]=(photo_path,bio_path,wordcloud_path,new_page_text_path)
    app = wx.App(False)
    frame = MainFrame()
    frame.Show()
    app.MainLoop()