...
# 通过得到的随机参数beta、pose、expression来重建人体模型mesh
def main(model_folder,
    ...
    # 保存网格模型到文件
    output_dir = osp.join(model_folder, '..', 'output_meshes_demo')  # 设置输出目录
    ...
    # 导出关节点的变换矩阵为 pkl 文件
    import os
    output_dir = osp.join(model_folder, '..', 'output_poses_demo')  # 设置输出目录
    os.makedirs(output_dir, exist_ok=True)  # 创建输出目录（如果不存在）

    import pickle  # 导入 pickle，用于序列化数据
    poses = {}
    poses['body_pose'] = output.body_pose.detach().cpu().numpy().reshape(1, -1)  # 提取身体姿态并展平
    print(poses['body_pose'].shape)  # 打印身体姿态的形状
