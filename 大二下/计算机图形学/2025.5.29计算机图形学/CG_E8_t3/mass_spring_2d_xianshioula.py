...
class Cloth:
    ...
    def compute_force(self):
        ...
        # 固定点受力设置为0
        self.force[self.N] = ti.Vector([0.0, 0.0])
        self.force[self.NV - 1] = ti.Vector([0.0, 0.0])

        # 加入阻尼力（速度方向的反向力）
        for i in self.vel:
            self.force[i] -= self.kd * self.vel[i]  # 阻尼力

    # 显式欧拉更新位置和速度
    @ti.kernel
    def updatePosVel(self, h: ti.f32):
        for i in self.pos:
            self.pos[i] += self.vel[i] * h  # 先更新位置，这时使用旧的速度
            self.vel[i] += (self.force[i] / self.mass[i]) * h  # 更新速度

    def update(self, h):
        self.compute_force()  # 计算受力
        self.updatePosVel(h)  # 使用显式欧拉法更新位置和速度
