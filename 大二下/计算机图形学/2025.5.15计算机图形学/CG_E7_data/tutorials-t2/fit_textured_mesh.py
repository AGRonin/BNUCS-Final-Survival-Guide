...
def visualize_prediction_multiview(mesh, renderer, cameras, iteration, outdir="results", silhouette=False):
    ...
    # 现在 grid 肯定不是 None
    plt.imsave(f"{outdir}/predicted_multiview_iter_{iteration}.png", np.clip(grid, 0, 1))
    plt.close('all')
...
for i in loop:
    ...
    if i % plot_period == 0:
        ...
        visualize_prediction_multiview(new_src_mesh, renderer_textured, target_cameras, iteration=i, silhouette=False)
...
visualize_prediction_multiview(new_src_mesh, renderer_textured, target_cameras, iteration=300, silhouette=False)
plot_losses(losses)
